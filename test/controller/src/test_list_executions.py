...
class TestParallelIndices(unittest.TestCase):
    ...
    def test_list_different_users(self):
        ...
        infos = context.controller.list_executions(
            first_user, list_for_all_users=False)
        self.assertSetEqual(
        ...
        infos = context.controller.list_executions(
            second_user, list_for_all_users=False)
        ...
        self.assertListEqual(
            [second_user_execution_id], [i['execution_id'] for i in infos])
        ...
        infos = context.controller.list_executions(
            first_user, list_for_all_users=True)
        self.assertSetEqual(
            {
        ...
        infos = context.controller.list_executions(
            second_user, list_for_all_users=True)
        self.assertSetEqual(
            {
                first_user_execution_id1,
                first_user_execution_id2,
