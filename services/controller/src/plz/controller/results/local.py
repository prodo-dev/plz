...
class LocalResultsStorage(ResultsStorage):
    def __init__(
            self, redis: StrictRedis, db_storage: DBStorage, directory: str):
    ...
    def publish(
        ...
        with self._lock(execution_id):
            ...
            _write_output_and_measures(
                paths, containers, execution_id, index_range_to_run)
...
class Paths:
    ...
    def output(self, subdir: Optional[str]) -> str:
        return os.path.join(
            self.directory, subdir if subdir is not None else '', 'output.tar')
