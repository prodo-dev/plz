...
def _setup_logging():
    ...
    if 'log_level' in config:
        ...
        print(
            f'Setting log level to: {log_level}', file=sys.stderr, flush=True)
...
def list_executions_entrypoint():
    ...
    return Response(
        json.dumps(
            {
                'executions':
                    controller.list_executions(user, list_for_all_users)
            }),
...
def get_logs_entrypoint(execution_id):
    ...
    return Response(
        controller.get_logs(execution_id, since=since),
...
def get_output_files_entrypoint(execution_id):
    ...
    return Response(
        controller.get_output_files(execution_id, path, index),
        mimetype='application/octet-stream')
